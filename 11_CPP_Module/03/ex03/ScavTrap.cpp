...
ScavTrap::ScavTrap(const ScavTrap& origin) : ClapTrap() {
  std::cout << kTraps[kScav] << kMessages[kCopyConstructor] << kSuffix[kMessage]
            << '\n';
  *this = origin;
}

ScavTrap& ScavTrap::operator=(const ScavTrap& origin) {
  std::cout << kTraps[kScav] << kMessages[kCopyAssignmentOperator]
            << kSuffix[kMessage] << '\n';
  if (this != &origin) {
...
ScavTrap::~ScavTrap() {
  std::cout << kTraps[kScav] << kMessages[kDestructor] << kSuffix[kMessage]
            << std::endl;
}
