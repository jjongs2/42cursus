...
class set {
 ...
 private:
  typedef ft::llrb<key_type, value_type, key_compare, allocator_type> llrb;

 public:
  typedef typename llrb::const_iterator iterator;
  typedef typename llrb::const_iterator const_iterator;
  typedef ft::reverse_iterator<iterator> reverse_iterator;
  typedef ft::reverse_iterator<const_iterator> const_reverse_iterator;

  set() : tree_(key_compare(), allocator_type()) {}
  ...
  set(const set& origin) : tree_(origin.tree_) {}
  ...
  key_compare key_comp(void) const { return tree_.key_comp(); }
  value_compare value_comp(void) const { return tree_.key_comp(); }

  template <typename K, typename Comp, typename Alloc>
  friend bool operator==(const set<K, Comp, Alloc>& lhs,
                         const set<K, Comp, Alloc>& rhs);
...
template <typename Key, typename Compare, typename Allocator>
void swap(set<Key, Compare, Allocator>& set1,
          set<Key, Compare, Allocator>& set2) {
  set1.swap(set2);
}
